...
* **규칙:** 비교하는 **두 값이 서로 다를 때만 1** 이 나오고, **같으면 0** 이 나옵니다.
...
💡 어떤 숫자들을 XOR 연산했을 때 결과가 `0`이 나오면 "에러 없음", `1`이 나오면 무언가 변형되었다는 뜻입니다!

---

### 2. 패리티 비트(Parity Bit)란?
오류가 생겼는지 검사하고 수정하기 위해 **기존 데이터에 추가하는 '체크용 덤 비트'** 입니다.

### 3. 해밍코드의 패리티 규칙
총 7비트 중 **1번, 2번, 4번**을 패리티 비트($P_1, P_2, P_3$)로 비워둡니다.
...
if len(data_input) != 4 or not all(c in '01' for c in data_input):
    ...
    * 계산된 패리티 비트: $P_1$=[ **{p1}** ], $P_2$=[ **{p2}** ], $P_3$=[ **{p3}** ]
    """)
    st.info(f"정해진 규칙대로 병합된 **7비트 송신 데이터**: `{encoded}`")
    st.markdown("---")
    ...
    # ⭐ [핵심 수정] 오류가 난 비트의 숫자 자리만 딱 골라서 볼드(강조) 문자열로 가공하는 과정
    highlighted_list = []
    for i, val in enumerate(corrupted):
        if i == (error_idx - 1):
            # 오류가 난 자리는 볼드(**) 처리하여 극대화
            highlighted_list.append(f"**{val}**")
        else:
            highlighted_list.append(str(val))
            
    # 대괄호로 묶어서 배열 모양처럼 예쁘게 출력
    visual_data = "[" + ", ".join(highlighted_list) + "]"
    
    st.warning(f"🚨 우주 방사선 피격! 현재 **{error_idx}번 비트** 가 전하 충격으로 반전되었습니다.")
    st.markdown(f"오류가 발생한 데이터 상태: {visual_data}") # 에러 비트만 볼드체로 출력됨!
    st.markdown("---")
    
    # 3. 에러 검출 및 복원 (디코딩)
    st.subheader("🔍 3단계: ECC 알고리즘 작동 (수학적 오류 복원)")
    st.markdown("수신 장치는 들어온 7비트 데이터를 다시 규칙대로 XOR 연산(신드롬 계산)하여 깨진 자리를 찾아냅니다.")
    
    # 수신된 데이터로 패리티 재계산 (신드롬 체크)
